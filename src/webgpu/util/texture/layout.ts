...
export function getTextureSubCopyLayout(
  ...
  let formatInfo: TextureFormatInfo = kTextureFormatInfo[format];
  if (formatInfo.depth || formatInfo.stencil) {
    const resolvedFormat = kDepthStencilFormatResolvedAspect[format as DepthStencilFormat][aspect];
    assert(resolvedFormat !== undefined);
    formatInfo = kTextureFormatInfo[resolvedFormat!];
    assert(formatInfo.copySrc);
  }
